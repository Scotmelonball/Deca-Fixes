...
def to_unsigned(value, bits=64):
    ...
    if value < 0:
        if bits == 32:
            ...
            # but ensure it's the correct negative value for comparison
            return value & ((1 << 64) - 1)
    return value
...
class Processor:
    ...
    def process_vhash_final(self, v_hash_in, db: DbWrap):
        ...
        # First convert the hash value appropriately
        if db.file_hash_type == node_flag_v_hash_type_4:
            v_hash = to_unsigned(v_hash_in, 32)
        elif db.file_hash_type == node_flag_v_hash_type_8:
            v_hash = to_unsigned(v_hash_in, 64)
        else:
            raise NotImplementedError('Unhandled Hash Type {}'.format(db.file_hash_type))

        # Use the converted hash value for the database queries
        nodes = db.db().nodes_where_match(v_hash=v_hash)

        # Get corresponding hash strings using the converted value
        if db.file_hash_type == node_flag_v_hash_type_4:
        ...
        for v_path in missed_vpaths:
            ...
            v_hash = to_unsigned(v_hash, 32 if db.file_hash_type == node_flag_v_hash_type_4 else 64)
